/* Dashboard: role based change traceability, notifications and logs. */
(function () {
  "use strict";
  ...
  var isDeveloper = session.role === "developer";
  var editingUid = null;
  ...
  document.getElementById("pw-form").addEventListener("submit", function (e) {
    ...
    var result = Store.changePassword(session.email, document.getElementById("pw-old").value, next);
    if (!result.ok) {
      msg.className = "msg err";
  ...
  function openReview(change, accepted) {
    ...
    document.getElementById("review-title").textContent =
      (accepted ? "Accept " : "Reject ") + change.changeId;
  ...
  document.getElementById("review-form").addEventListener("submit", function (e) {
    e.preventDefault();
    var change = Store.changes().filter(function (c) { return c.uid === reviewState.uid; })[0];
    if (!change || change.testerEmail !== session.email) { reviewModal.hidden = true; return; }

    var accepted = reviewState.accepted;
    Store.updateChange(change.uid, {
      status: accepted ? "Accepted" : "Rejected",
      reviewedAt: new Date().toISOString(),
      testerComment: document.getElementById("review-comment").value.trim()
    });
    Store.audit(accepted ? "Change accepted" : "Change rejected", change.changeId, session.email);
    reviewModal.hidden = true;
  ...
  /* ---------- developer form ---------- */
  function fillTesters() {
    if (!el.tester) return;
    var testers = Store.testers();
  ...
  if (isDeveloper) {
    document.getElementById("change-form").addEventListener("submit", function (e) {
      ...
      var tester = Store.findUser(testerEmail);
      var payload = {
      ...
      if (editingUid) {
        ...
        Store.updateChange(editingUid, payload);
        Store.audit("Change edited", payload.changeId, session.email);
        exitEditMode();
        el.formMsg.className = "msg ok";
        el.formMsg.textContent = "Change request updated. Use Notify Tester to send it again.";
      } else {
        Store.addChange(payload);
        e.target.reset();
        el.formMsg.className = "msg ok";
  ...
  function cardHtml(c) {
    ...
    return '<article class="change">' +
      '<div class="chips">' +
        ...
        '<span class="chip st-' + esc(c.status.replace(/\s/g, "")) + '">' + esc(c.status) + "</span>" +
  ...
  el.list.addEventListener("click", function (e) {
    ...
    var change = Store.changes().filter(function (c) { return c.uid === uid; })[0];
    if (!change) return;
    ...
    if (btn.dataset.act === "notify") {
      if (change.developerEmail !== session.email) return;
      Store.updateChange(uid, { status: "Tester Notified", notifiedAt: new Date().toISOString() });
    ...
    if (btn.dataset.act === "mail") {
      if (change.developerEmail !== session.email) return;
      Store.updateChange(uid, { status: "Tester Notified", notifiedAt: new Date().toISOString() });
      Store.audit("Tester mailed", change.changeId + " sent to " + change.testerEmail, session.email);
  ...
  function renderStats() {
    var mine = Store.changes();
    var by = function (s) { return mine.filter(function (c) { return c.status === s; }).length; };
  ...
  if (!isDeveloper) {
    Store.changes()
      .filter(function (c) { return c.testerEmail === session.email && c.status === "Tester Notified"; })
      .slice(0, 3)
      .forEach(function (c) {
